...
function Get-PowerCode
{
    ...
    # If provided, will get PowerCode that extend a given command
    [Parameter(ValueFromPipelineByPropertyName)]
    ...
    If the extension has an Alias with a regular expression literal (```'/Expression/'```) then the -PowerCodeName will be valid if that regular expression matches.
    #>
    [Parameter(ValueFromPipelineByPropertyName)]
    ...
    If provided, will treat -PowerCodeName as a wildcard.
    This will return any extension whose name, displayname, or aliases are like the -PowerCodeName.
    ...
    If provided, will treat -PowerCodeName as a regular expression.
    This will return any extension whose name, displayname, or aliases match the -PowerCodeName.
    ...
    # If set, will return the dynamic parameters object of all the PowerCode for a given command.
    [Parameter(ValueFromPipelineByPropertyName)]
    [switch]
    $DynamicParameter,
    ...
    # If set, will return the dynamic parameters of all PowerCode for a given command, using the provided DynamicParameterSetName.
    ...
    # If provided, will return the dynamic parameters of all PowerCode for a given command, with all positional parameters offset.
    # Implies -DynamicParameter.
    [Parameter(ValueFromPipelineByPropertyName)]
    [int]
    $DynamicParameterPositionOffset = 0,

    # If set, will return the dynamic parameters of all PowerCode for a given command, with all mandatory parameters marked as optional.
    ...
    begin {
        $PowerCodePattern = '\.powercode\.ps1$'
        $PowerCodeTypeName = 'PipeScript.Transpiler'
        ...
        function ConvertToExtension($toExtension) {
            ...
            $extCmd =
                ...
                elseif ($in -is [IO.FileInfo]) {
                    if ($in.LastWriteTime -gt $script:PowerCodesFileTimes[$in.Fullname]) {
                        $script:PowerCodesFileTimes[$in.Fullname] = $in.LastWriteTime
                        $script:PowerCodesFromFiles[$in.Fullname] = 
                            $ExecutionContext.SessionState.InvokeCommand.GetCommand($in.fullname, 'ExternalScript,Application')
                        $script:PowerCodesFromFiles[$in.Fullname]
                    } elseif ($script:PowerCodesFromFiles[$in.Fullname])  {
                        return $script:PowerCodesFromFiles[$in.Fullname]
                    }                    
                }
                else {
            ...
            if (-not $PowerCodeTypeName) {
                $PowerCodeTypeName = 'Extension'
            }
            if ($extCmd.pstypenames -notcontains $PowerCodeTypeName) {            
                $extCmd.pstypenames.insert(0,$PowerCodeTypeName)
            }
        ...
        $extensionFullRegex =
            [Regex]::New($(
                if ($PowerCodeModule) {
                    "\.(?>$(@(@($PowerCodeModule) + $PowerCodeModuleAlias) -join '|'))\." + "(?>$($PowerCodePattern -join '|'))"
                } else {
                    "(?>$($PowerCodePattern -join '|'))"
                }
        ...
        $myInv = $MyInvocation
        $myModuleName = if ($PowerCodeModule) { $PowerCodeModule } else { $MyInvocation.MyCommand.Module.Name }
        ...
        if ($Force) {
            $script:PowerCodes  = $null
            $script:PowerCodesByName    = $null
            $script:AllCommands = @()
        }
        if (-not $script:PowerCodes)
        {
            $script:PowerCodesFromFiles     = [Ordered]@{}
            $script:PowerCodesFileTimes     = [Ordered]@{}
            $script:PowerCodesByName        = [Ordered]@{}
            $script:PowerCodesByDisplayName = [Ordered]@{}
            $script:PowerCodesByPattern     = [Ordered]@{}
            $script:PowerCodes =
                @(@(
                #region Find PowerCode in Loaded Modules
                ...
                #endregion Find PowerCode in Loaded Modules

                #region Find PowerCode in Loaded Commands                
                $ExecutionContext.SessionState.InvokeCommand.GetCommands('*', 'Function,Alias',$true) -match $extensionFullRegex
                #endregion Find PowerCode in Loaded Commands
                ) | Select-Object -Unique | Sort-Object Rank, Name)

            foreach ($extCmd in $script:PowerCodes) {
                if (-not $script:PowerCodesByName[$extCmd.Name]) {
                    $script:PowerCodesByName[$extCmd.Name] = $extCmd
                }
                else {
                    $script:PowerCodesByName[$extCmd.Name] = @($script:PowerCodesByName[$extCmd.Name]) + $extCmd
                }
                if ($extCmd.DisplayName) {
                    if (-not $script:PowerCodesByDisplayName[$extCmd.DisplayName]) {
                        $script:PowerCodesByDisplayName[$extCmd.DisplayName] = $extCmd
                    }
                    else {
                        $script:PowerCodesByDisplayName[$extCmd.DisplayName] = @($script:PowerCodesByDisplayName[$extCmd.DisplayName]) + $extCmd
                    }   
                }
                ...
                if ($ExtensionCommandAliasRegexes) {
                    foreach ($extensionAliasRegex in $ExtensionCommandAliasRegexes) {
                        ...
                        if (-not $script:PowerCodesByPattern[$regex]) {
                            $script:PowerCodesByPattern[$regex] = $extCmd
                        } else {
                            $script:PowerCodesByPattern[$regex] = @($script:PowerCodesByPattern[$regex]) + $extCmd
                        }
                    }
                }
                if ($ExtensionCommandNormalAliases) {
                    foreach ($extensionAlias in $ExtensionCommandNormalAliases) {
                        if (-not $script:PowerCodesByName[$extensionAlias]) {
                            $script:PowerCodesByName[$extensionAlias] = $extCmd
                        } else {
                            $script:PowerCodesByName[$extensionAlias] = @($script:PowerCodesByName[$extensionAlias]) + $extCmd
                        }
                    }
                }
    ...
    process {
        ...
        if ($PowerCodePath) {
            @(foreach ($_ in Get-ChildItem -Recurse:$($PowerCodePath -notmatch '^\.[\\/]') -Path $PowerCodePath -File) {
                if ($_.Name -notmatch $extensionFullRegex) { continue }
                if ($CommandName -or $PowerCodeName) {
        ...
        } elseif ($CommandName -or $PowerCodeName) {
            if (-not $CommandName -and -not $like -and -not $Match) {
                foreach ($exn in $PowerCodeName) {
                    if ($script:PowerCodesByName[$exn]) {
                        $script:PowerCodesByName[$exn] | OutputExtension
                    }
                    if ($script:PowerCodesByDisplayName[$exn]) {
                        $script:PowerCodesByDisplayName[$exn] | OutputExtension
                    }
                    if ($script:PowerCodesByPattern.Count) {
                        foreach ($patternAndValue in $script:PowerCodesByPattern.GetEnumerator()) {
                            if ($patternAndValue.Key.IsMatch($exn)) {
                                $patternAndValue.Value | OutputExtension
                            }
                        }
                        $script:PowerCodesByDisplayName[$exn]
                    }
                }                
            } else {
                $script:PowerCodes |
